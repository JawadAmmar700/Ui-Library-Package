...
export function AnimatedButtonComponent({
  ...
  return (
    <div className="relative group overflow-hidden text-white px-4 py-1 bg-white/30  rounded-lg shadow-lg  flex-shrink-0">
      ...
      <p className="relative z-10 font-semibold text-xs md:text-sm truncate select-none">
        {item}
      </p>
      <button
        onClick={() => onClick(item)}
        className="absolute inset-0 flex items-center justify-center z-20 font-bold text-black bg-white rounded-lg transform -translate-x-full group-hover:translate-x-0 transition-transform duration-500 ease-in-out group-hover:shadow-xl"
